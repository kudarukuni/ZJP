...
Class Action {
	...
	function save_recruitment_status(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO enrollment_status SET ".$data);
		}else{
			$save = $this->db->query("UPDATE enrollment_status SET '.$data.' WHERE id = ".$id);
		}
		if($save)
			return 1;
	}
	function delete_recruitment_status(){
		extract($_POST);
		$delete = $this->db->query("UPDATE enrollment_status SET status = 0 WHERE id = ".$id);
		if($delete)
			return 1;
	}
